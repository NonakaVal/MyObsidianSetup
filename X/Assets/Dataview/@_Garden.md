🫘Seed | 🌱 Plant | ☘️ Cultivate 	🪴 Repot | 🍄 Question | 💦 Revitalize | 🍁 Revisit
~~~dataviewjs
//-----------------------------------------------------
// CONFIGURAÇÃO — GARDEN
//-----------------------------------------------------
...
function getGardenIcon(tags) {
  if (!tags) return "";
  const arr = Array.isArray(tags) ? tags : [tags];
  for (const [tag, icon] of Object.entries(GARDEN_ICONS)) {
    const bare = tag.replace("#", "");
    if (arr.some(t => t === tag || t === bare)) return icon + " ";
  }
  return "";
}
...
function getGardenTags(tags) {
  if (!tags) return "";
  const arr = Array.isArray(tags) ? tags : [tags];
  return arr.filter(t => t.startsWith("#garden/") || t.startsWith("garden/")).join(", ");
}

//-----------------------------------------------------
// COLETA
//-----------------------------------------------------
const pages = dv.pages("#garden")
  .where(p =>
    !p.file.path.includes("30 Knowlegde/35 Recources/Ideaverse Pro 2") &&
    !p.file.path.includes("System") &&
    !p.file.name.includes("Master Key (Garden Tags)")
  )
...
pages.array().forEach(p => {
  ...
  const prefix = getGardenIcon(p.file.tags);
  const tagStr = getGardenTags(p.file.tags);
  ...
  `;
  row.onmouseenter = () => row.style.background = "#0f1a12";
