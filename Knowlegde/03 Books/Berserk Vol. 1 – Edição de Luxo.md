---
tags:
  - book
aliases:
title: "Berserk: Vol 1 – Edição de Luxo"
subtitle: ""
author:
  - Kentaro Miura
genres:
  - Mangá, Ação & Fantasia
publisher: Panini Brasil
published: "2021"
volume: "224"
isbn: "9788542617092"
cover: https://m.media-amazon.com/images/I/51J9tn7W3jL._SY300_.jpg
localCover: ""
status: Finished
dateCreated: "[[2025-10-20]]"
cssclasses:
  - imagegallery
---


%% To use an image URL from the server, use the following syntax: %%


%% To save images locally, enable the 'Enable Cover Image Save' option in the settings a nd enter as follows: %%


# Berserk: Vol 1 – Edição de Luxo


# Páginas de destaque ⭐

![img|200](https://imgur.com/4d2rHXA.png)
![img|200](https://imgur.com/GOHafL2.png)
# Principais Aprendizados 📚
_Resuma os principais pontos ou lições que você aprendeu com o livro._
- 
- 
- 

# Citações Favoritas 💬
_Anote trechos ou citações que mais ressoaram com você._
- 
- 
- 

# Reflexões Pessoais ✨
_Compartilhe seus pensamentos, opiniões e como o livro o impactou._
- 

# Avaliação ⭐
_Atribua uma nota ao livro com base na sua experiência pessoal (ex.: 1–5 estrelas)._
- 

# Próximo Objetivo de Leitura 📖
_Defina uma intenção para a sua próxima jornada de leitura._
- 


## Reading Journey

- 2025-10-20: Finished