---
created: "[[2025-07-10]]"
share_link: https://share.note.sx/0i9hh31d#pdi2BWiu7B9jAMLvGWCQ1OQn/1VR6R/v3DWkfO6pfHg
share_updated: 2025-07-10T19:14:57-03:00
---
# O Sistema ACE: Estruturando Seu Pensamento

## Visão Geral do ACE

- **`+`**: Pasta de captura inicial
- **`x`**: Pasta de utilitários

## Os Três Espaços Mentais

| Espaço       | Foco          | Dimensão Temporal | Intenção  | Princípio Organizador |
|-------------|--------------|------------------|----------|----------------------|
| **Atlas**   | Conhecimento | Atemporal        | Compreender | Espaço (relações) |
| **Calendar**| Tempo        | Temporal         | Focar    | Sequência temporal |
| **Efforts** | Ação         | Oportuno         | Agir     | Prioridade/urgência |


> "A estrutura deve ser conquistada, não imposta" - Evite planejamentos excessivos que podem se tornar frágeis na prática.

> [[Organização das pastas]]


← Parte de [[LYT - Concepts]]